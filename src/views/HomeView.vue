...
onMounted(async () => {
  const response = await axios.get('https://api.systembitz.com/reviews.json', {params: {page: 1}});
  if(response) {
    reviews.value = response.data
  }
  const res = await axios.get('https://api.systembitz.com/ferienhaus-cape-coral');
  if(response) {
